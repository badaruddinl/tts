...
function applyHumanLikeProsodyLimiter(units, opts = {}) {
  ...
  const strength = clamp(Number(opts.prosodyLimiterStrength ?? 0.72), 0.3, 1);
  ...
  const absRate = expressive ? 26 : 22;
  const absPitch = expressive ? 16 : 13.5;
  const absVolume = expressive ? 15 : 12;
  const stepRate = (2.2 + intensity * 1.1) * strength + 0.5;
  const stepPitch = (1.6 + intensity * 0.9) * strength + 0.4;
  const stepVolume = (1.5 + intensity * 0.8) * strength + 0.4;
    ...
    JSON.stringify(
      {
        ...
        prosodyLimiter: {
          ...
          strength: clamp(Number(prosodyLimiterStrength ?? 0.72), 0.3, 1)
        },
