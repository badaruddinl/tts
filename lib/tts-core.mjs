...
import { loadJsonFromSqlite, sqliteConfigEnabled } from "./runtime-config-bridge.mjs";

const require = createRequire(import.meta.url);
const VOICE_PRESET_PATH = path.resolve(process.cwd(), "config", "voice-character", "presets.json");
const TEXT_LEXICON_PATH = path.resolve(process.cwd(), "config", "text", "lexicon.json");
...
export function cleanLine(line) {
  ...
  // Accept scene control tags with common variants/typos and optional payload.
  out = out.replace(/\[SCENE(?:N)?(?:\s*:[^\]]+)?\]/gi, "");
  // Drop other bracket control tags so they are not spoken,
  // but keep intent tags because they are parsed later by parseIntentTag().
  out = out.replace(/\[([^\]]+)\]/g, (full, inner) => {
    const raw = String(inner || "").trim();
    if (!raw) return "";
    const key = raw.split(/[\s:=,|]/)[0]?.toLowerCase() || "";
    if (key === "intent" || key === "emo" || key === "emotion") return full;
    return "";
  });
...
function loadTextLexicon(filePath = TEXT_LEXICON_PATH) {
  const target = path.resolve(String(filePath || TEXT_LEXICON_PATH));
  if (!fs.existsSync(target)) return [];
  try {
    const raw = JSON.parse(fs.readFileSync(target, "utf8"));
    const rows = Array.isArray(raw?.replacements)
      ? raw.replacements
      : raw && typeof raw === "object"
        ? Object.entries(raw).map(([from, to]) => ({ from, to }))
        : [];
    return rows
      .map((item) => ({
...
function applyLexiconRewrite(text, lexiconRows) {
  let out = String(text || "");
  for (const item of lexiconRows || []) {
    ...
    out = out.replace(pattern, (full, left) => `${left}${to}`);
  }
  return out;
}

function autoPunctuateLine(line, cfg = {}) {
  ...
  const qStarters = new Set(
    Array.isArray(cfg.questionStarters)
      ? cfg.questionStarters.map((v) => String(v).toLowerCase())
      : ["apa", "apakah", "siapa", "kapan", "di mana", "dimana", "kenapa", "mengapa", "bagaimana", "bisakah"]
  );
  const xStarters = new Set(
    Array.isArray(cfg.exclaimStarters)
      ? cfg.exclaimStarters.map((v) => String(v).toLowerCase())
      : ["wow", "astaga", "waduh", "ya ampun", "gila", "hebat"]
  );

  const minWordsForComma = Number.isFinite(Number(cfg.minWordsForComma))
    ? Math.max(8, Math.floor(Number(cfg.minWordsForComma)))
    : 14;
  const commaConnectors = new Set(
    Array.isArray(cfg.commaConnectors)
      ? cfg.commaConnectors.map((v) => String(v).toLowerCase())
      : ["tapi", "namun", "lalu", "kemudian", "dan", "sementara", "sedangkan"]
  );

  let candidate = text;
  if (!candidate.includes(",") && words.length >= minWordsForComma) {
    const target = Math.floor(words.length / 2);
    let best = -1;
    let bestDist = Number.POSITIVE_INFINITY;
    for (let i = 2; i < words.length - 2; i += 1) {
      const token = words[i].replace(/[^\p{L}\p{N}_-]/gu, "");
      if (!commaConnectors.has(token)) continue;
    ...
    if (best > 1) {
      const rawWords = candidate.split(/\s+/);
      rawWords[best] = `, ${rawWords[best]}`;
      candidate = rawWords.join(" ").replace(/\s+,/g, ",");
    }
  }

  const startsWithQuestion = qStarters.has(words[0] || "") || /\b(kah)\b/.test(lower);
  const startsWithExclaim = xStarters.has(words[0] || "") || /^[A-Z0-9\s]{6,}$/.test(text);
  if (startsWithQuestion) return `${candidate}?`;
  if (startsWithExclaim) return `${candidate}!`;
  return `${candidate}.`;
}

function resolveAutoPunctuateConfig(mode, baseCfg = {}) {
  const key = String(mode || "").trim().toLowerCase();
  const out = { ...(baseCfg || {}) };
  if (key === "conservative") {
    out.minWordsForComma = Number.isFinite(Number(out.minWordsForComma)) ? Number(out.minWordsForComma) : 16;
    out.questionStarters = Array.isArray(out.questionStarters)
      ? out.questionStarters
      : ["apa", "apakah", "siapa", "kapan", "di mana", "dimana", "kenapa", "mengapa", "bagaimana"];
    out.exclaimStarters = Array.isArray(out.exclaimStarters) ? out.exclaimStarters : ["astaga", "waduh"];
    return out;
  }
  if (key === "aggressive") {
    out.minWordsForComma = Number.isFinite(Number(out.minWordsForComma)) ? Number(out.minWordsForComma) : 10;
    out.questionStarters = Array.isArray(out.questionStarters)
      ? out.questionStarters
      : [
          "apa",
    ...
    out.exclaimStarters = Array.isArray(out.exclaimStarters)
      ? out.exclaimStarters
      : ["wow", "astaga", "waduh", "ya ampun", "gila", "hebat"];
    return out;
  }
  out.minWordsForComma = Number.isFinite(Number(out.minWordsForComma)) ? Number(out.minWordsForComma) : 14;
  return out;
}

function autoPunctuateText(text, cfg = {}) {
  return String(text || "")
    .split(/\r?\n/)
    .map((line) => autoPunctuateLine(line, cfg))
    .filter(Boolean)
    .join("\n");
}

function preprocessNarrationText(rawText, opts = {}) {
  const autoPunctuate = toBool(opts.autoPunctuate ?? process.env.TTS_AUTO_PUNCTUATE, true);
  const textRewrite = toBool(opts.textRewrite ?? process.env.TTS_TEXT_REWRITE, true);
  const autoPunctuateMode = String(opts.autoPunctuateMode ?? process.env.TTS_AUTO_PUNCTUATE_MODE ?? "balanced");
  const lexiconPath = String(opts.textLexiconPath || process.env.TTS_TEXT_LEXICON || TEXT_LEXICON_PATH);
  const punctCfgRaw = opts.punctuateConfig && typeof opts.punctuateConfig === "object" ? opts.punctuateConfig : {};
  const punctCfg = resolveAutoPunctuateConfig(autoPunctuateMode, punctCfgRaw);

  let out = String(rawText || "").replace(/\s+\n/g, "\n").trim();
  if (!out) return out;
  if (textRewrite) {
    const lexicon = loadTextLexicon(lexiconPath);
    out = applyLexiconRewrite(out, lexicon);
  }
  if (autoPunctuate) {
    out = autoPunctuateText(out, punctCfg);
  }
  return out;
}
...
function readVoicePresetFile() {
  if (sqliteConfigEnabled()) {
    const fromDb = loadJsonFromSqlite("config/voice-character/presets.json");
...
function splitSegmentIntoPhrases(text, cfg) {
  ...
  const words = toWordTokens(raw);
  if (words.length <= cfg.maxWords + 2) return [raw].filter(Boolean);

  const commaClauses = raw.split(/,\s+/).map((s) => s.trim()).filter(Boolean);
  if (commaClauses.length > 1) {
    const validClauses = commaClauses.every((c) => {
      const n = toWordTokens(c).length;
      return n >= 1 && n <= cfg.maxWords + 2;
    });
    if (validClauses) {
      return commaClauses.map((c, i) => (i < commaClauses.length - 1 ? `${c},` : c)).filter(Boolean);
    }
  }
...
function splitIntoSegments(text) {
  ...
  const segments = [];
  for (const line of lines) {
    // Keep commas inside a segment to avoid unnatural pauses in Indonesian storytelling.
    const parts = line
      .split(/(?<=[.!?;:])\s+/)
      .map((item) => item.trim())
      .filter(Boolean)
...
export function previewAutoExpressionFromText({
  ...
  textRewrite = true,
  textLexiconPath = null
}) {
  const preparedText = preprocessNarrationText(text, {
    autoPunctuate,
    autoPunctuateMode,
    textRewrite,
    textLexiconPath
  });
  const segments = splitIntoSegments(preparedText);
  ...
  built.segments = applyHumanLikeProsodyLimiter(built.segments, {
    humanizeIntensity,
...
function applyHumanLikeProsodyLimiter(units, opts = {}) {
  if (!Array.isArray(units) || units.length === 0) return [];
  const intensity = clamp(Number(opts.humanizeIntensity ?? 0.45), 0, 1);
  const enabled = opts.prosodyLimiter !== false;
  if (!enabled) return units.map((u) => ({ ...u }));
  const strength = clamp(Number(opts.prosodyLimiterStrength ?? 0.64), 0.3, 1);
  const style = String(opts.style || "").toLowerCase();
  const expressive =
    style.includes("dram") || style.includes("thriller") || style.includes("sinem") || style.includes("tegang");

  // Softer limiter envelope to keep natural dynamics while preventing spikes.
  const absRate = expressive ? 30 : 27;
  const absPitch = expressive ? 18 : 16.5;
  const absVolume = expressive ? 16 : 14;
  const softness = 0.75 + strength * 0.25;
  const stepRate = (2.8 + intensity * 1.3) * softness + 0.55;
  const stepPitch = (2.0 + intensity * 1.0) * softness + 0.45;
  const stepVolume = (1.8 + intensity * 0.9) * softness + 0.45;

  const out = [];
  let prev = null;
  for (const row of units) {
    const next = { ...row };
    next.rate = clamp(next.rate, -absRate, absRate);
    next.pitch = clamp(next.pitch, -absPitch, absPitch);
    next.volume = clamp(next.volume, -absVolume, absVolume);

    if (prev) {
      next.rate = prev.rate + clamp(next.rate - prev.rate, -stepRate, stepRate);
      next.pitch = prev.pitch + clamp(next.pitch - prev.pitch, -stepPitch, stepPitch);
      next.volume = prev.volume + clamp(next.volume - prev.volume, -stepVolume, stepVolume);
    }
    out.push(next);
    prev = next;
  }
...
export async function synthesizeToMp3({
  ...
  const preparedText = preprocessText
    ? preprocessNarrationText(text, {
        autoPunctuate,
        autoPunctuateMode,
        textRewrite,
        textLexiconPath
  ...
  fs.writeFileSync(tempInputPath, preparedText, "utf8");
...
export async function synthesizeHumanizedToMp3({
  ...
  const proposed = prosodyMap.map((seg, i) => {
    const mlAdj = policy
    ...
    return {
      seg,
      mlAdj,
      rate: clamp(seg.rate + mlAdj.dr, -35, 35),
      pitch: clamp(seg.pitch + mlAdj.dp, -20, 20),
      volume: clamp(seg.volume + mlAdj.dv, -20, 20)
    };
  });
  const limited = applyHumanLikeProsodyLimiter(
    proposed.map((p) => ({ rate: p.rate, pitch: p.pitch, volume: p.volume })),
    {
      humanizeIntensity,
      style: built.styleName,
      prosodyLimiter,
  ...
  const units = proposed.map((p, i) => ({
    seg: p.seg,
    mlAdj: p.mlAdj,
    final: limited[i] || { rate: p.rate, pitch: p.pitch, volume: p.volume }
  }));
  const segmentPaths = await mapWithConcurrency(
    units,
    async (unit) => {
      const { seg, mlAdj, final } = unit;
      const segBase = path.join(segDir, `seg_${String(seg.index).padStart(4, "0")}`);
      const segOut = await synthesizeToMp3({
        text: seg.text,
        output: segBase,
        voice,
        rate: `${final.rate.toFixed(0)}%`,
        pitch: `${final.pitch.toFixed(0)}Hz`,
        volume: `${final.volume.toFixed(0)}%`,
        cacheDir: workCacheDir,
        backend,
  ...
  let mapPath = null;
  if (saveProsody) {
    mapPath = path.resolve(`${normalizedOutput}.prosody.json`);
    fs.writeFileSync(
      mapPath,
      JSON.stringify(
        {
          ...
          prosodyLimiter: {
            enabled: Boolean(prosodyLimiter),
            strength: clamp(Number(prosodyLimiterStrength ?? 0.64), 0.3, 1)
          },
