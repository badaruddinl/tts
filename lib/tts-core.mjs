...
function resolveStyleConfig(styleName, profileFileOverride = null) {
  ...
  const selected = styleName && styles[styleName] ? styleName : defaultStyle;
...
function detectIntentBoost(text, idx, total, styleName) {
  const t = String(text || "").toLowerCase().trim();
  const isOpening = idx === 0;
  const isShort = t.length <= 20;
  const isGreeting = /^(hai|halo|hello|helo|hi|apa kabar|selamat (pagi|siang|sore|malam))\b/i.test(t);
  const cheerful = /\b(asyik|senang|bahagia|akhirnya|mantap|hebat|seru|keren|yes|yey|hore)\b/i.test(t);
  const assertive = /\b(dengar|perhatikan|ingat|harus|wajib|sekarang)\b/i.test(t);
  const sad = /\b(sedih|kecewa|hampa|lelah|menangis|sunyi)\b/i.test(t);
  const tense = /\b(gelap|langkah|pintu|bayangan|bisik|takut|mencekam)\b/i.test(t);
  ...
  if (isGreeting && isOpening && isShort) {
    // Make plain "hai" sound warm/expressive without requiring "haiiii".
    rate += 1.4;
    pitch += 2.2;
    volume += 0.8;
    intent = "greeting_warm";
  }
  if (cheerful) {
    rate += 1.2;
    pitch += 1.8;
    volume += 0.9;
    intent = intent === "neutral" ? "cheerful" : `${intent}+cheerful`;
  }
  if (assertive) {
    rate += 0.8;
    pitch -= 0.6;
    volume += 1.1;
    intent = intent === "neutral" ? "assertive" : `${intent}+assertive`;
  }
  if (sad) {
    rate -= 1.6;
    pitch -= 1.4;
    volume -= 0.8;
    intent = intent === "neutral" ? "sad" : `${intent}+sad`;
  }
  if (tense || styleName === "horor_intim" || styleName === "misteri") {
    rate -= 0.9;
    pitch -= 0.8;
    volume += 0.3;
    if (intent === "neutral") intent = "tense";
  }
  ...
  return { rate, pitch, volume, intent };
}
...
function buildProsodyMap(segments, opts) {
  ...
  const segmentsOut = segments.map((text, i) => {
    ...
    const ib = autoExpressive ? detectIntentBoost(text, i, segments.length, styleCfg.styleName) : null;
    if (ib) {
      rate += ib.rate * intensity;
      pitch += ib.pitch * intensity;
      volume += ib.volume * intensity;
    }
