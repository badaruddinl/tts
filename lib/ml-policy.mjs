...
export function loadPolicyModelFromPath(modelPath) {
  ...
  if (payload?.modelType === "linear_py_v1") {
    return {
      meta: payload.meta,
      modelType: "linear_py_v1",
      model: {
        weights: Array.isArray(payload?.weights) ? payload.weights : [],
        intercept: Array.isArray(payload?.intercept) ? payload.intercept : [0, 0, 0]
      }
    };
  }
  return {
...
export function predictAdjustment(policy, context) {
  ...
  if (policy.modelType === "linear_py_v1") {
    ...
    const dim = Math.min(feat.length, weights.length);
    for (let i = 0; i < dim; i += 1) {
      const row = Array.isArray(weights[i]) ? weights[i] : [0, 0, 0];
      const x = Number(feat[i] || 0);
    ...
    pred = policy.model.predict(feat);
  }
